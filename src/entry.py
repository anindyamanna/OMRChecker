...
def process_single_image(omr_file, set_layout=False):
    template = None
    tuning_config = CONFIG_DEFAULTS
    curr_dir = Path().resolve()
    # Update local tuning_config (in current recursion stack)
    local_config_path = curr_dir.joinpath("OMRChecker", constants.CONFIG_FILENAME)
    print(local_config_path)
    if os.path.exists(local_config_path):
        tuning_config = open_config_with_defaults(local_config_path)

    # Update local template (in current recursion stack)
    local_template_path = curr_dir.joinpath("OMRChecker", "infinitiq_template.json")
    local_template_exists = os.path.exists(local_template_path)
    if local_template_exists:
        template = Template(
            local_template_path,
    ...
    output_dir = Path("outputs", curr_dir.relative_to(Path().resolve()))
    paths = Paths(output_dir)
    ...
    local_evaluation_path = curr_dir.joinpath(constants.EVALUATION_FILENAME)
    evaluation_config = None
    if not set_layout and os.path.exists(local_evaluation_path):
        if not local_template_exists:
            logger.warning(
                f"Found an evaluation file without a parent template file: {local_evaluation_path}"
            )
    ...
    # if args["setLayout"]:
    if set_layout:
        show_template_layouts([omr_file], template, tuning_config, direct_omr_file=True)
    else:
...
def show_template_layouts(omr_files, template, tuning_config, direct_omr_file=False):
    for file_path in omr_files:
        ...
        template_layout = template.image_instance_ops.draw_template_layout(
            in_omr, template, shifted=True, border=2
        )
        InteractionUtils.show(
            f"Template Layout: {file_name}", template_layout, 1, True, config=tuning_config
        )


class OMRException(Exception):
    def __str__(self):
        return "Please try again. Ensure the full omr sheet is visible and the image has enough light."
    ...
    for file_path in omr_files:
        ...
        if single_file:
            ...
            for field_block in template.field_blocks:
                if field_block.name in template.custom_labels.keys():
                    omr_dict[field_block.name] = omr_response[field_block.name]
                else:
                    new_items = {i: omr_response[i] for i in field_block.parsed_field_labels}
                    if field_block.merged_field_name:
                        omr_dict[field_block.merged_field_name] = {**omr_dict.get(field_block.merged_field_name, {}), **new_items}
                    else:
                        omr_dict[field_block.name] = new_items
            return omr_dict
        if (
